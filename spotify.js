        ...
        newlist.innerHTML += `
            <div style="display:flex;">
                ...
                <div style="display: flex; align-items: center; margin-left: auto;">
                    ...
                    <div>
                        <button style="background: transparent;" onclick="playsongs('${encodeURIComponent(song)}', '${currentFolder}')">
...
function loadfavsonglist() {
    ...
    for (const song of yourfavoritesongs) {
        let partofsongnamearr = song.split('/'); // Split URL by "/"
        let partIndex = partofsongnamearr.indexOf('songs') + 1; // Find 'songs' and get next part
        let folder = partofsongnamearr[partIndex];// Extract folder name
        if (folder) {
            let temp = song.split(`songs/${folder}/`)[1].replaceAll('%20', ' '); // Extract song name
            let newfavlist = document.createElement('li');
            newfavlist.innerHTML = `
                <div style="display:flex;">
                    ...
                    <div style="display: flex; align-items: center; margin-left: auto;">
                        ...
                        <div>
                            <button style="background: transparent;" onclick="playsongs('${encodeURIComponent(song)}', '${folder}')">
                                <img src="playicon.png" alt="play" height="50px" style="cursor: pointer;"/>
                            </button>
                        </div>
                    </div>
...
heart.addEventListener("click", () => {
    if (!(yourfavoritesongs.includes(currentsong.src))) { // If song is not already in favorites
        ...
        loadfavsonglist(currentFolder); // Use the stored folder name
